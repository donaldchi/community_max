...
char *filename_coe  = NULL; //ceofficient file name
int type       = UNWEIGHTED;
int nb_pass    = 0;
...
usage(char *prog_name, const char *more) {
  cerr << more;
  cerr << "usage: " << prog_name << " input_file [-w weight_file] [-p part_file] [-q epsilon] [-c coefficient_file] [-l display_level] [-v] [-h]" << endl << endl;
  ...
  cerr << "-c file\tread graph node`s coefficient files!" << endl;
...
main(int argc, char **argv) {
  ...
  Community c(filename, filename_w, filename_coe, type, -1, precision);
  ...
  do {
    ...
    clock_t start_level, finish_level;
    start_level  = clock();
    improvement  = c.one_level(level, filename);
    finish_level = clock();
    cerr<<"level "<<level<<":: "<<(double)(finish_level-start_level)/CLOCKS_PER_SEC<<"sec."<<endl;
  ...
  if (verbose) {
    ...
    cerr << "Total duration: " << duration << " sec." << endl;
  }
  cerr << new_mod << endl;
}
